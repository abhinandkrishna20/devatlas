...
const FeaturePost = ({ mainPost, sidePosts }: FeaturePostProps) => {
  ...
  return (
    <Card>
      ...
      <div className="feature-post-container  flex flex-col md:flex-row gap-6 p-4 ">
        ...
        <div className="feature-post-main  flex-1 flex flex-col gap-3">
          ...
          <div className="post-content mt-2">
            <h2 className="text-2xl md:text-3xl font-bold  cursor-pointer hover:text-amber-800 transition-colors leading-tight">
              {mainPost.title}
            </h2>
            <div className="flex items-center gap-2 mt-2 text-xs md:text-sm  font-medium">
              <span className="text-gray-200">{mainPost.author}</span>
              <span>•</span>
        ...
        <div className="feature-post-side w-full md:w-[32%] flex flex-col gap-5 ">
          {sidePosts.slice(0, 2).map((post) => (
            <div key={post.id} className="side-post flex flex-col gap-2 group cursor-pointer">
              ...
              <h3 className="text-md font-bold text-gray-200 group-hover:text-amber-800 transition-colors line-clamp-2 leading-snug">
                {post.title}
              </h3>
              </NavLink>
            </div>
